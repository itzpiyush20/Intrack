...
**`@capacitor/core` and `@capacitor/haptics` are the runtime dependencies** —
`LandingPage.tsx` imports `@capacitor/core` to detect the native shell, and
`src/components/ui/haptics.ts` imports `@capacitor/haptics` on demand, only on
native, for the tap/success/warning ticks used by the motion kit.
`@capacitor/cli`, `@capacitor/android` and `@capacitor/ios` are
**devDependencies**, and must stay there. They are
